...
public class JwtTokenUtils {
    ...
    public static Admin getCurrentUser() {
        ...
        try {
            ...
            // 解析token，获取用户的id
            String adminId = JWT.decode(token).getAudience().get(0);
            Admin admin = staticAdminService.findById(Integer.valueOf(adminId));
            if (admin == null) {
                log.error("トークンに関連する管理者情報が見つかりません。トークン={}", token);
                throw new CustomException("トークンに関連するユーザ情報が見つかりません。再ログインしてください。");
            }
            return admin;
        } catch (CustomException ce) {
            throw ce; // 已经是 CustomException 类型的异常，直接抛出
